        ...
        async initAdminGroup () {
            ...
            if (email) {
                ...
                if (!user.isPersistant()) {
                    ...
                    const events = [];
                    // apply command
                    events.push(user.apply(new UserWithPWARegistered({ userId, email: this.email, passwordHash: hashedPassword, locale })));
            ...
            if (!group.isPersistant()) {
                ...
                const events = [];
                // apply command
                events.push(group.apply(new GroupCreated({ groupId, label: this.uniqueKey })));
