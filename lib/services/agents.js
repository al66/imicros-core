...
const { 
 ...
 const { AgentRepository } = require("../classes/repositories/repositories");
 ...
 module.exports = {
    ...
    actions: {
        ...
        create: {
          ...
          async handler (ctx) {
             const events = [];
            // validate command
            // const { decoded } = await this.getUser(ctx);
            const { decoded: access } = await this.getAccess(ctx);
            if (!access.groupId) throw new UnvalidToken({ token: ctx.meta?.accessToken });
            const agentId = ctx.params.agentId;
            if (access.role !== "admin") throw new RequiresAdminRole({ groupId: access.groupId, role: access.role }); 
            const agent = await this.agentRepository.getById({ uid: agentId });
            if (agent.isPersistant()) throw new AgentAlreadyExists({ uid: agentId });
            // apply command
            events.push(agent.apply(new AgentCreated({ groupId: access.groupId, agentId, label: ctx.params.label })));
            // persist
            await agent.commit();
             // return result
             return true;
       ...
       rename: {
          ...
          async handler (ctx) {
             ...
             const { decoded: access } = await this.getAccess(ctx);
             if (access.role !== "admin") throw new RequiresAdminRole({ groupId: access.groupId }); 
             const agentId = ctx.params.agentId;
             const agent = await this.agentRepository.getById({ uid: agentId });
             if (access.groupId !== agent.getGroupId()) throw new UnvalidToken({ token: ctx.meta?.accessToken });
             if (!agent.isPersistant()) throw new AgentDoesNotExist({ uid: agentId });
             // apply command
             events.push(agent.apply(new AgentRenamed({ groupId: access.groupId, agentId, label: ctx.params.label })));
             // persist
             await agent.commit();
              // return result
              return true;
        ...
        */
        createCredentials: {
            ...
            async handler (ctx) {
                ...
                if (!agent.isPersistant()) throw new AgentDoesNotExist({ uid: agentId });
                // apply command
                events.push(agent.apply(new CredentialsCreated({ groupId: access.groupId, agentId, credentialsId, credentials })));
                // persist
                await agent.commit();
                 // return result
                 return true;
        ...
        */
        deleteCredentials: {
            ...
            async handler (ctx) {
                ...
                // apply command
                events.push(agent.apply(new CredentialsDeleted({ groupId: access.groupId, agentId, credentialsId })));
                // persist
                await agent.commit();
                 // return result
                 return true;
        ...
        */
        delete: {
            ...
            async handler (ctx) {
                ...
                events.push(agent.apply(new AgentDeleted({ groupId: access.groupId, agentId, deletedBy: user.uid })));
        ...
        */
        get: {
            ...
            async handler (ctx) {
                ...
                // const { user } = await this.getUser(ctx);
                const { decoded: access } = await this.getAccess(ctx);
                if (access.role !== "member" && access.role !== "admin") throw new RequiresUnrestrictedAccess({ groupId: access.groupId }); 
                const agentId = ctx.params.agentId;
                const agent = await this.agentRepository.getById({ uid: agentId });
                if (access.groupId !== agent.getGroupId()) throw new UnvalidToken({ token: ctx.meta?.accessToken });
        ...
        verifyAuthToken: {
            ...
            async handler(ctx) {
                ...
                const { token: agentTokenSigned } = await this.encryption.sign({ payload: { 
                    type: Constants.TOKEN_TYPE_AGENT, 
                    agentId: agent.getId(), 
    ...
    methods: {
        ...
        async getUser(ctx) {
            const decoded = await this.encryption.verify({ token: ctx.meta?.accessToken || ctx.meta?.userToken });
