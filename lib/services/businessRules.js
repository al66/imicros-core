    ...
    actions: {
        ...
        get: {
            ...
            async handler(ctx) {
                if (!ctx.meta.ownerId) throw new Error("No owner id in context");
                if (!ctx.meta.accessToken) throw new Error("No access token in context");
                return this.db.get({ owner: ctx.meta.ownerId, accessToken: ctx.meta.accessToken, businessRuleId: ctx.params.businessRuleId, xml: true });
            }
        },
        ...
        evaluate: {
            ...
            async handler(ctx) {
                if (!ctx.meta.ownerId) throw new Error("No owner id in context");
                if (!ctx.meta.accessToken) throw new Error("No access token in context");
                const decision =  this.db.get({ owner: ctx.meta.ownerId, accessToken: ctx.meta.accessToken, businessRuleId: ctx.params.businessRuleId });
                if (!decision?.ast) throw new Error("Decision not found");
                return this.evaluate({ ast: decision.ast, context: ctx.params.context });
            }
        },
