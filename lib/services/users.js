    ...
    actions: {
        ...
        requestConfirmation: {
            async handler(ctx) {
                ...
                // apply command
                events.push(user.apply(new UserConfirmationRequested({ userId: user.state.uid, confirmationToken: confirmationTokenSigned.token, email: user.getEmail(), locale: user.getLocale() })));
                // persist
                await user.commit();
                // return result
                return true;
