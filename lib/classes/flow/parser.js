...
class Parser {
    ...
    parseEventName({ from, to, platform }) {
        let name;
        switch (platform) {
            case Constants.PLATFORM_CAMUNDA:
                to.attributes.name = this.toArray(from.extensionElements.properties.property).find((p) => p._name === "eventName")?._value || null;
                break;
            case Constants.PLATFORM_FLOW:
                to.attributes.name = from.extensionElements?.startEvent?._eventName || null
        };
    }
    ...
    parseTasks({ from, to, subprocess }) {
        ...
        this.toArray(from.businessRuleTask).forEach((e) => {
            ...
            this.toArray(e.extensionElements?.ioMapping?.output).forEach((output) => {
    ...
    parseEvents({ from, to, subprocess }) {
        ...
        this.toArray(from.startEvent).forEach((e) => {
            ...
            this.parseEventName({ from: e, to: event, platform: to.process.platform });
            ...
            this.toArray(e.extensionElements?.ioMapping?.output).forEach((out) => {
                let output = {
                    _source: out._source,
                    _target: out._target
                };
                if (output._source.startsWith("=")) {
                    let success = this.feel.parse(output._source.substr(1));
                    if (success) {
                        output._ast = this.feel.ast;
                    }
                }
                event.output.push(output);
            });
            to.event.push(event);
        });
    ...
    parse({ id, xmlData, objectName, ownerId, core }) {
        ...
        // console.log(util.inspect(parsedProcess, { showHidden: false, depth: null, colors: true }));
