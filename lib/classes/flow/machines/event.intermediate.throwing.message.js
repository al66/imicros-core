...
const MessageIntermediateThrowingMachine = {
    id: "event",
    initial: "idle",
    context: ({ input }) => ({ 
    ...
    entry: [
        sendParent(({ context }) => ({ 
            type: "log", data: { message: "Intermediate Throwing Message Event created (M)",  data: { elementId: context.element.id, position: context.element.position, data: context.data } 
        }}))
    ],
    states: {
        idle: {
            on: {
                activate: [{
                    ...
                    actions: [
                        ...
                        sendParent(({ context }) => ({ type: "log", data: { message: "Schedule job for sending message",  data: { elementId: context.element.id, type: context.element.type } }}))
                    ]
                }]
            }
        },
...
module.exports = {
    MessageIntermediateThrowingMachine
};
