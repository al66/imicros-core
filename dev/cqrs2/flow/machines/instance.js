    ...
    states: {
        ...
        active: {
            onRaiseEvent: async ({ command, self }) => {
                // search in events
                let element = self.context.processData.event.find(event => event.localId === command.eventId);
                if (element) {
                    // raise event
            ...
            onAddContext: async ({ command, self }) => {
                const value = self.application.evaluate({ expression: command.expression, context: command.context });
            ...
            onActivateNext: ({ command, self }) => {
                ...
                next = next.map(id => {
                    ...
                    if (element) {
                        return self.execute(new ActivateElement({ instanceId: self.uid, id: element.id, machine: GatewayMachine, element }));  
            ...
            onContextAdded: ({ event, self }) => {
                self.context.data[event.key] = event.value;
            }
        },
        finished: {}
    }
